...
int main( int argc, char **argv ) {
  ...
  THREADED(g.num_openmp_processes)
  {
    ...
    for(g.time_slice = 0; g.time_slice< g.global_lattice[0][0]-1; g.time_slice++){
      
      if(g.my_rank==0) printf("g.global_lattice[0][0] = %d \t  timeslice %d\n", g.global_lattice[0][0], g.time_slice);
